...
async def _configured_zhipu_vision_analysis(image_url: str, user_prompt: str) -> str:
    """Use the configured zhipu-vision MCP implementation as a fast bridge.

    Telegram image preprocessing runs before the agent can choose an MCP tool.
    When the main OpenAI-compatible endpoint is text-only, routing that image
    through the generic auxiliary client waits for a timeout. Reuse the exact
    implementation and credential already configured for the zhipu MCP server
    so automatic preprocessing and manual ``vision_analyze`` calls share the
    working vision backend.

    ``LookupError`` means the bridge is not configured and callers may use the
    normal auxiliary router. Other exceptions mean the configured backend
    failed and should be surfaced immediately instead of falling into a known
    text-only endpoint and blocking the Telegram turn for minutes.
    """
    from hermes_cli.config import load_config

    cfg = load_config()
    server_cfg = (cfg.get("mcp_servers") or {}).get("zhipu-vision") or {}
    env_cfg = server_cfg.get("env") or {}
    api_key = str(env_cfg.get("ZHIPU_API_KEY") or "").strip()
    ...
    if isinstance(args, str):
        try:
            ...
            args = [args]
    server_path = next(
        (str(item) for item in args if str(item).endswith("server.py")),
        "/root/.hermes/mcp-servers/zhipu-vision/server.py",
    )
    if not api_key or not os.path.isfile(server_path):
        raise LookupError("zhipu-vision MCP bridge is not configured")

    def _call() -> str:
        spec = importlib.util.spec_from_file_location(
            f"hermes_zhipu_vision_{uuid.uuid4().hex}", server_path
        )
        if spec is None or spec.loader is None:
            raise RuntimeError(f"cannot load zhipu vision server: {server_path}")
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        return module.describe_image(image_url, user_prompt, api_key=api_key)
...
async def vision_analyze_tool(
    ...
    # Prefer the explicitly configured visual MCP backend. The current main
    # endpoint (local DeepSeek-web proxy) is text-only; sending image_url input
    # to it merely waits for the auxiliary timeout and wedges Telegram replies.
    try:
        analysis = await _configured_zhipu_vision_analysis(image_url, user_prompt)
        return json.dumps({"success": True, "analysis": analysis}, indent=2, ensure_ascii=False)
    except LookupError:
        pass
    except Exception as exc:
        logger.error("Configured zhipu vision bridge failed: %s", exc)
        return json.dumps(
            {"success": False, "analysis": f"智谱视觉识别失败: {exc}"},
            indent=2,
            ensure_ascii=False,
        )
